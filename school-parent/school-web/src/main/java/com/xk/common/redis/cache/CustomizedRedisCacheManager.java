...
public class CustomizedRedisCacheManager extends RedisCacheManager {
    ...
    {
        this(RedisCacheWriter.nonLockingRedisCacheWriter(connectionFactory), RedisCacheConfiguration.defaultCacheConfig().entryTtl(Duration.ofSeconds(30)),
                cacheItemConfigList
                        .stream()
    ...
    public CustomizedRedisCacheManager(RedisCacheWriter redisCacheWriter, RedisCacheConfiguration redisCacheConfiguration, Map<String, RedisCacheConfiguration> redisCacheConfigurationMap)
    {
...
//        RedisCache missingCache = getMissingCache(name);
        CacheItemConfig cacheItemConfig = CacheContainer.getCacheItemConfigByCacheName(name);
        CustomizedRedisCache redisCache = new CustomizedRedisCache(name, this.redisCacheWriter, RedisCacheConfiguration.defaultCacheConfig().entryTtl(Duration.ofSeconds(cacheItemConfig.getExpiryTimeSecond())), this.redisOperations);
        return redisCache;
    }
